...
import { loadServers, saveServers } from "./config.js";
...
class ServerInstance {
  ...
  async start() {
    ...
    this.client = new Client({ name: "dsh-mcp-connector", version: "0.1.0" }, { capabilities: {} });
    this.client.onclose = () => {
      this.connected = false;
  ...
  async syncTools() {
    ...
    do {
      ...
      for (const tool of response.tools) {
        ...
        if (definitions.has(publicName)) {
          throw new Error(`mcp-connector(${this.server.serverName}): duplicate tool "${tool.name}"`);
        }
          ...
          output: {
            ...
            render(_args, value) {
              if (Array.isArray(value?.content)) {
                return value.content.map((b) => (b.type === "text" ? { type: "text", text: b.text } : { type: "text", text: JSON.stringify(b) }));
              }
              return [{ type: "text", text: JSON.stringify(value) }];
            }
          },
          execute: createExecutor(this.client, tool.name, this.server.toolCallTimeoutMs ?? 60000)
        });
      }
    ...
    try {
      for (const [name, def] of definitions) {
        this.disposers.set(name, this.ctx.tools.register(def));
      }
    } catch (e) {
      for (const dispose of this.disposers.values()) dispose();
      this.disposers.clear();
...
export default {
  name: "mcp-connector",
  inject: ["tools", "subprocess", "fs", "harness", "timer", "webServer"],
  async apply(ctx) {
    const data = loadServers();
    const instances = new Map(); // id -> ServerInstance
    const logs = [];
    const MAX_LOGS = 500;
    ...
    for (const server of data.servers) {
      ...
      try {
        ...
        // connection failed, will retry via user action
      }
    }
    ...
    register("mcp/add-server", async (args) => {
      ...
      const instance = new ServerInstance(ctx, server);
      instances.set(server.id, instance);
      try { await instance.start(); } catch (e) { ctx.logger.error(`mcp-connector: failed to start new server: ${e.message}`); }
    ...
    register("mcp/remove-server", async (args) => {
      ...
      if (instance) { await instance.stop(); instances.delete(id); }
    ...
    register("mcp/toggle-server", async (args) => {
      ...
      if (enabled) {
        ...
        try { await instance.start(); } catch (e) { ctx.logger.error(`mcp-connector: failed to start server: ${e.message}`); }
      } else {
        const instance = instances.get(id);
        if (instance) { await instance.stop(); instances.delete(id); }
      }
    ...
    register("mcp/restart-server", async (args) => {
      ...
      try { await newInstance.start(); } catch (e) { ctx.logger.error(`mcp-connector: restart failed: ${e.message}`); }
      return { ok: true };
    });
    ...
    // Serve management UI at /mcp and API under /mcp/api/*
    ...
    const disposeRoute = ctx.get("webServer").register({
      ...
      handler: async (req, res) => {
        ...
        if (pathname.startsWith("/api/")) {
          ...
          let body;
          try { body = await readBody(); } catch { json(400, { error: "Invalid JSON" }); return; }
